...
void scan_loop(void) {
  ...
  if (scanstatus == 2) {
    ...
    EEPROM.writeUInt(EE_UINT16T_SCANNERVBW, scanner_vbw);
    EEPROM.writeByte(EE_UINT8T_SCANNERTH, scanner_th);
    EEPROM.writeByte(EE_UINT8T_SCANNERBAND, scanner_band);
    EEPROM.writeByte(EE_UINT8T_SCANNERSPEED, scanner_speed);
    EEPROM.writeByte(EE_UINT8T_SCANNERTHENABLE, scanner_thenable);
    EEPROM.commit();
